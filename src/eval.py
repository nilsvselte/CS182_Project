...
def build_evals(conf):
    ...
    if conf.training.curriculum_type == 'gradual_subspace_increase':
        n_points = conf.training.curriculum.points.end
    elif conf.training.curriculum_type == 'standard':
        n_points = conf.training.n_points

    batch_size = conf.training.batch_size

    prompt_type = conf.training.prompt_type
    result_type = conf.training.result_type
    weight_type = conf.training.result_type

    if conf.training.task_type == 'single_task':
        task_name = conf.training.task
    elif conf.training.task_type == 'multi_task':
        task_name = conf.training.task_kwargs['task_1']['task']
